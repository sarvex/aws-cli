...
def apply_boolean_options(
    ...
    if true_option and false_option:
        error_message = f'aws: error: cannot use both {true_option_name} and {false_option_name} options together.'
...
def find_master_public_dns(session, parsed_globals, cluster_id):
    ...
    return "" if master_instance is None else master_instance.get('PublicDnsName')
