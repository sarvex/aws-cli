...
COMPLETIONS = [
    ...
    (
        'aws sts decode-authorization-message --encoded-message --re',
        -1,
        {'--region'},
    ),
    (
        'aws sts decode-authorization-message --encoded-message --enco',
        -1,
        set([]),
    ),
    ('aws ec2 --debug describe-instances --instance-ids ', -1, set([])),
    (
        'aws ec2 --debug describe-instances --instance-ids i-12345678 - ',
        -1,
        {
            '--filters',
            '--dry-run',
    ...
    ('aws s3', -1, {'cp', 'mv', 'rm', 'mb', 'rb', 'ls', 'sync', 'website'}),
    ('aws s3 m', -1, {'mv', 'mb'}),
    (
        'aws s3 cp -',
        -1,
